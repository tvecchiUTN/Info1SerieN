...
void *my_bsearchMod(const void *key, const void *base, size_t nitems, size_t size, int (*compar)(const void *, const void *), int (*opcion)(const void *))
{
    ...
    for(size_t i = 0; i < nitems; i++)
    {
        ...
        if(compar && compar(key, puntero_elemento)) 
        {
            return puntero_elemento;
        }
        if(opcion && opcion(puntero_elemento))
        {
            return puntero_elemento;
        }
    }
